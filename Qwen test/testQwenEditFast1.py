...
def enable_sdpa():
    try:
        # Prefer the new SDPA kernel API when available (fast profile kept)
...
def main():
    ...
    log_mem("after model load")  # added
    ...
    log_mem("after LoRA attach")  # added
    ...
    for idx, (hair_text, out_path) in enumerate(hair_variants, start=1):
        ...
        if torch.cuda.is_available():
            try:
                ...
                pass
        log_mem(f"before gen {idx}")  # added
